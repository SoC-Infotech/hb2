...
pygame.display.set_caption("Hangman - by Caleb")
...
def redraw_game_window():
    ...
    #add score label
    score_label = guess_font.render("Score: ", 1 , BLACK)   
    win.blit(score_label, (winWidth/2 + 100, winHeight/2 - 100))
    #pygame.display.flip()

    #add hint above the word
    hint_label = guess_font.render("Hint: " + hint, 1 , BLACK)
    win.blit(hint_label, (winWidth/2 - 210, winHeight - 100))
...
def randomWord():
    file = open('words.txt')
    file_hint = open('hints.txt')
    f = file.readlines()
    f_hint = file_hint.readlines()
    i = random.randrange(0, len(f) - 1)

    return f[i][:-1], f_hint[i][:-1]
...
# Function to display cumulative score of games won and lost
def score():
    print(" ")
    print(" Score")
    print(" -----")
...
#add score label
score_label = guess_font.render("SCORE: 0", 1 , BLACK)   
win.blit(score_label, (winWidth/2, winHeight/2))
pygame.display.flip()
